...
public:
	...
	/**
	 ensures add = #x + y
	 */
	friend std::tuple<natural_number, natural_number> add(natural_number&& x, natural_number&& y) {
	 ...
	 */
	friend std::tuple<natural_number, natural_number> subtract(natural_number&& x, natural_number&& y) {
	 ...
	 */
	friend std::tuple<natural_number, natural_number> multiply(natural_number&& x, natural_number&& y) {
		...
		std::tie(product_casted, y_casted) = multiply(std::move(product_casted), std::move(y_casted));

		y.rep_ = std::move(y_casted);
		product.rep_ = std::move(product_casted);

		return std::make_tuple(std::move(product), std::move(y));
	}
};
